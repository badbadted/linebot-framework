...
async function handleList(ctx) {
  try {
    ...
    return ctx.reply('⚠️ 查詢活動時發生錯誤，請稍後再試');
  }
}
...
async function handleInfo(ctx) {
  ...
  if (!keyword) {
    return ctx.reply('請輸入活動名稱\n範例：/nj_info 親子露營');
  }

  try {
    const event = await findEventByTitle(db, keyword);
    if (!event) {
      return ctx.reply(`找不到「${keyword}」相關活動 🤔\n輸入 /nj 查看目前活動列表`);
    }
    ...
    return ctx.reply('⚠️ 查詢活動詳情時發生錯誤，請稍後再試');
  }
}
...
async function handleJoin(ctx) {
  ...
  try {
    ...
    // 檢查狀態
    if (!['voting', 'upcoming'].includes(event.status)) {
      return ctx.reply(`「${event.title}」目前${formatEventStatus(event.status)}，無法報名`);
    }

    // 檢查人數上限
    if (event.maxParticipants && event.currentParticipants >= event.maxParticipants) {
      return ctx.reply(`「${event.title}」已額滿（${event.currentParticipants}/${event.maxParticipants}）😢`);
    }

    // 檢查是否已報名
    const already = await hasJoined(db, event.id, userId);
    if (already) {
      return ctx.reply(`你已經報名「${event.title}」了 ✅\n輸入 /nj_my 查看你的活動`);
    }

    // 取得使用者資料
    const user = await getUser(db, userId);
    if (!user) {
      return ctx.reply('尚未在妞揪平台建立帳號\n請先從 LINE 選單開啟妞揪 App 登入一次');
    }
    ...
    return ctx.reply('⚠️ 報名時發生錯誤，請稍後再試');
  }
}
...
async function handleStats(ctx) {
  try {
    ...
    return ctx.reply('⚠️ 查詢統計時發生錯誤，請稍後再試');
  }
}
